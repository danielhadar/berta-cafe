...
var ACTIVE_TAB_STORAGE_KEY = "berta_active_tab";
...
var USER_CODE_STORAGE_KEY = "berta_user_code";
var CODE_LENGTH = 6;
...
// stored locally under "berta_remote_<code>". Deploy backend/Code.gs and paste
// the URL here to enable real cross-device sync.
var BACKEND_URL = "https://script.google.com/macros/s/AKfycbyvkDbeA-uzOBYRHTM_sHoBgVx2YkAcWWLfVy_azEnVlJnxBMv_YmT5INd0BJEWxiV6rA/exec";
...
function backendGet(code) {
  if (!BACKEND_URL) {
    try {
      var raw = localStorage.getItem("berta_remote_" + code);
...
function backendSet(code, state) {
  if (!BACKEND_URL) {
    try { localStorage.setItem("berta_remote_" + code, JSON.stringify(state)); } catch (e) {}
    return;
  }
